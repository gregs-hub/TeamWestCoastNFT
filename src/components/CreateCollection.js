...
const CreateCollection = ({ state, account, setCollection, collection }) => {
    ...
    const createCollection = async () => {
      ...
      const deployThenList = async (result) => {
        ...
        if (NFT == "SFT") {
          const instance = await(await state.factorySFTContract.createSFTCollection(uri, hexaRandomString)).wait();
          const addr = instance.to;
          await(await state.marketContract.addCollection(addr, uri, true)).wait();
        } else {
          const instance = await(await state.factoryContract.createNFTCollection(artistName, artistSymbol, uri, hexaRandomString)).wait();
          const addr = instance.to;
